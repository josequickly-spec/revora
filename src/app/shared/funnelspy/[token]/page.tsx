  ...
  return (
    <main className="min-h-screen bg-[#07090f] px-5 py-14 text-slate-100">
      <div className="mx-auto max-w-5xl">
        <div className="flex items-center justify-between gap-4"><div className="text-xs font-black uppercase tracking-[.2em] text-violet-300">FunnelSpy · Shared report</div><FunnelSpyPrintButton /></div>
        <h1 className="mt-4 text-5xl font-black">{audit.domain}</h1>
        <p className="mt-3 text-slate-500">{new Date(audit.createdAt).toLocaleString("en-US")}</p>
        <section className="mt-8 grid gap-4 md:grid-cols-4">
          {[["Score", audit.analysis.score], ["Pages", audit.analysis.totals.pages], ["CTAs", audit.analysis.totals.ctas], ["Forms", audit.analysis.totals.forms]].map(([label, value]) => (
