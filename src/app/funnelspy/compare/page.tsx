...
export default function FunnelSpyComparePage() {
  ...
  async function compare() {
    ...
    if (urls.length < 2) return setError("Add at least two domains.");
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || "Could not compare.");
      setAnalyses(data.analyses); setWinner(data.winner);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not compare."); }
    ...
    <main className="min-h-screen bg-[#07090f] px-5 py-12 text-slate-100">
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-8 rounded-3xl border border-violet-400/20 bg-gradient-to-br from-violet-500/10 to-cyan-400/[.04] p-7">
          <div className="flex items-center gap-2 text-xs font-black uppercase tracking-[.18em] text-violet-300"><Swords className="size-4" />Battlecard</div><h1 className="mt-3 text-4xl font-black">Compare competitors</h1><p className="mt-3 max-w-2xl text-slate-400">Enter between two and five domains, one per line. The analysis compares structure, capture, tracking, and performance.</p>
          ...
          <button onClick={compare} disabled={loading} className="mt-4 flex items-center gap-2 rounded-xl bg-white px-5 py-3 font-black text-slate-950 disabled:opacity-50">{loading ? <LoaderCircle className="size-4 animate-spin" /> : <Swords className="size-4" />}{loading ? "Comparing sites..." : "Create comparison"}</button>
        ...
        {analyses.length > 0 && <section className="mt-8 overflow-x-auto rounded-3xl border border-white/[.08]">
          <div className="grid min-w-[760px]" style={{ gridTemplateColumns: `180px repeat(${analyses.length}, minmax(180px, 1fr))` }}>
            <div className="border-b border-white/[.08] bg-white/[.03] p-5 text-xs uppercase text-slate-500">Dimension</div>
            ...
            {[
              ...
              ["Pages", (item: FunnelSpyAnalysis) => item.totals.pages],
              ["CTAs", (item: FunnelSpyAnalysis) => item.totals.ctas],
              ["Forms", (item: FunnelSpyAnalysis) => item.totals.forms],
              ["Pixels", (item: FunnelSpyAnalysis) => item.totals.pixels],
