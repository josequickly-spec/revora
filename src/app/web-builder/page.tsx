...
const starterSpec: WebBuilderSpec = {
  ...
  previewDisclaimer: "Preview only · No business claims, orders or leads are stored here",
};
...
export default function AIWebBuilderPage() {
  ...
  const [context, setContext] = useState<Partial<WebBuilderRequest> & { businessId?: number }>({});
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      if (autoFlowRef.current && businessId) {
        router.push(`/businesses/${businessId}/consultant?auditId=${auditId || ""}&flow=1`);
      }
    } catch (error) {
  ...
  }, [audience, businessName, context.auditId, context.businessId, context.currentPrice, context.evidence, context.otomSummary, context.primaryObjective, context.recommendations, context.valueProposition, context.visualIdentity, context.weaknesses, offer, router, sourceUrl, businessType]);

  useEffect(() => {
    if (!autoFlowRef.current || autoGeneratedRef.current || generating) return;
    if (!businessName.trim() && !sourceUrl.trim()) return;
    autoGeneratedRef.current = true;
    const timer = window.setTimeout(() => {
      void generate();
    }, 0);
    return () => window.clearTimeout(timer);
  }, [businessName, generate, generating, sourceUrl]);
    ...
    <div className="min-h-screen overflow-x-hidden bg-[#070707] text-slate-100 selection:bg-orange-500/30">
      ...
      <main className="relative z-10 mx-auto max-w-[1600px] px-5 py-8">
        ...
        {view !== "revenue" ? <div className={`grid gap-6 ${view === "build" ? "xl:grid-cols-[330px_1fr]" : ""}`}>
          ...
          <section className="rounded-3xl border border-white/[.08] bg-white/[.025] p-3 md:p-6"><div className="mb-4 flex items-center justify-between px-1"><div><span className="text-[10px] font-black uppercase tracking-[.16em] text-slate-600">Live render</span><strong className="mt-1 block text-sm">{spec.brand.name}</strong></div><span className="rounded-full border border-orange-400/20 bg-orange-400/[.07] px-3 py-1 text-[9px] font-black uppercase tracking-wider text-orange-300">Client preview</span></div><WebBuilderPreview spec={spec} device={device} /></section>
