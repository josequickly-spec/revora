...
const schema=z.object({name:z.string().trim().min(1).max(160),jobType:z.string().min(3).max(100),cronExpression:z.string().min(5).max(100),timezone:z.string().min(1).max(100),payload:z.record(z.string(),z.unknown()).default({}),nextRunAt:z.string().datetime().nullable().default(null)}).strict();
