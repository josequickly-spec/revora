...
export async function processOutreachJob(payload: { body: Record<string, any>; requestUrl?: string }) {
 const body = payload.body;
 const origin = payload.requestUrl ? new URL(payload.requestUrl).origin : undefined;
 ...
 const computedPreviewUrl = body.previewUrl || (funnel.slug && origin ? `${origin}/en/funnel/${funnel.slug}` : undefined);
 ...
 previewDescription,
 previewAvailable: Boolean(computedPreviewUrl),
