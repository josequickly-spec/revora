...
export function ConsultantDetailView({ reportId }: { reportId: string }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <Card><div className="flex flex-wrap gap-3"><Link href={`/businesses/${record.businessId}/consultant?auditId=${record.auditId}`} className="rounded-xl bg-cyan-300 px-4 py-2 text-sm font-black text-slate-950">Regenerate with review</Link><button disabled title="Available in Phase 5" className="rounded-xl border border-white/10 px-4 py-2 text-sm text-slate-500">Create Proposal · Phase 5</button></div></Card>
    </div>
  );
}
